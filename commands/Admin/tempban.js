...
module.exports = {
   ...
   let bl = await db.fetch(`blid_${message.author.id}`)
    ...
    if (message.guild.id === n) return message.channel.send("Error, your guild is blacklisted.")
      ...
      if (!message.member.hasPermission('BAN_MEMBERS')) return message.author.send(":no_entry_sign: Nope, not enough perms");
  ...
  if(message.author.id === process.env.id){
    ...
    if (ms(tempBanTime)) {
      ...
      let TempBannedEmbed = new RichEmbed()
      ...
      let tempbanchannel = message.guild.channels.find('name', `logs`);

      if (!tempbanchannel) return message.channel.send(":warning: The admins on the server didnt make **logs** make sure to contact the owner or admins on the server for this problem. :warning:")
      ...
      setTimeout(function () {
        ...
        if (!unbanchannel) return message.channel.send(":warning: The admins on the server didnt make **logs** make sure to contact the owner or admins on the server for this problem. :warning:");
